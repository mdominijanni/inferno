...
class TestLinearConnections:
    ...
    class TestDenseConnection:
        ...
        def test_update_weight_add(self, connection):
            update = torch.rand_like(connection.weight)
            res_true = connection.weight + update
            connection.update_weight_add(update)
        ...
        class TestDirectConnection:
            ...
            def test_update_weight_add(self, connection, mask):
                update = torch.rand_like(connection.weight)
                res_true = mask * (connection.weight + update)
                connection.update_weight_add(update)
                res_test = connection.weight
                assert torch.all(res_test == res_true)
