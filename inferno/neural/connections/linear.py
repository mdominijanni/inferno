...
class DenseConnection(AbstractConnection):
    ...
    def __init__(
        ...
        # register weights
        self.register_parameter('_weight', nn.Parameter(torch.empty((self._output_size, self._input_size)), False))
...
class DirectConnection(AbstractConnection):
    ...
    def __init__(
        ...
        # register weights
        self.register_parameter('_weight', nn.Parameter(torch.empty((self._size, self._size)), False))
        weight_init(self._weight)

        # masking matrix (diagonal is 1, rest is 0)
        self._mask = torch.eye(self.size, dtype=self.weight.dtype, device=self.weight.device)
