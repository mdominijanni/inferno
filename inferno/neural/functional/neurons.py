...
def _voltage_thresholding_continuous(
    inputs: torch.Tensor,
    ...
    Args:
        ...
        thresh_v (float | torch.Tensor): membrane voltage at which action potentials are generated,
            in :math:`\mathrm{mV}`.
        abs_refrac (float | torch.Tensor): number of time steps the absolute refractory period lasts,
            in :math:`\mathrm{ms}`.
    ...
    Returns:
        tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
            ...
            - refracs -  amount of remaining time needed to exit refractory periods, in (:math:`ms`).
    """
    ...
    refracs = refracs.where(~spikes, abs_refrac)
...
def _voltage_thresholding_slope_intercept_discrete(
    inputs: torch.Tensor,
    refracs: torch.Tensor,
    voltage_fn: Callable[[torch.Tensor], torch.Tensor],
    ...
    r"""Manage refractory periods, spiking, and voltage based on voltage thresholding,
    tracking refractory periods as integer steps.
...
def _voltage_thresholding_slope_intercept_continuous(
    inputs: torch.Tensor,
    refracs: torch.Tensor,
    voltage_fn: Callable[[torch.Tensor], torch.Tensor],
    step_time: float | torch.Tensor,
