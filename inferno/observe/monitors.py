...
class InputMonitor(Monitor):
    ...
    Note:
        ...
        If there is only one input tensor, this will work as expected. Otherwise a ``mapping``
        must be specified which takes the unpacked inputs and returns a single tensor.
    """
...
class OutputMonitor(Monitor):
    ...
    Note:
        The output depends on the :py:meth:`~torch.nn.Module.forward` of the :py:class:`~torch.nn.Module`
        being called. If it a single tensor, it will work as expected. Otherwise a ``mapping``
        must be specified which takes the output and returns a single tensor.
    """
    ...
    def __init__(
        ...
        eval_update: bool = True,
        prepend: bool = False,
        mapping: Callable[[Any], torch.Tensor]
        | None = None,
    ):
        # determine arguments for superclass constructor
        if mapping:
